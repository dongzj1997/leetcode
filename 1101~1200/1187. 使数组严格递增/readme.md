# 1187. 使数组严格递增

```c++
给你两个整数数组 arr1 和 arr2，返回使 arr1 严格递增所需要的最小「操作」数（可能为 0）。

每一步「操作」中，你可以分别从 arr1 和 arr2 中各选出一个索引，分别为 i 和 j，0 <= i < arr1.length 和 0 <= j < arr2.length，然后进行赋值运算 arr1[i] = arr2[j]。

如果无法让 arr1 严格递增，请返回 -1。

 

示例 1：

输入：arr1 = [1,5,3,6,7], arr2 = [1,3,2,4]
输出：1
解释：用 2 来替换 5，之后 arr1 = [1, 2, 3, 6, 7]。
示例 2：

输入：arr1 = [1,5,3,6,7], arr2 = [4,3,1]
输出：2
解释：用 3 来替换 5，然后用 4 来替换 3，得到 arr1 = [1, 3, 4, 6, 7]。
示例 3：

输入：arr1 = [1,5,3,6,7], arr2 = [1,6,3,3]
输出：-1
解释：无法使 arr1 严格递增。
 

提示：

1 <= arr1.length, arr2.length <= 2000
0 <= arr1[i], arr2[i] <= 10^9

来源：力扣（LeetCode）
链接：https://leetcode-cn.com/problems/make-array-strictly-increasing
著作权归领扣网络所有。商业转载请联系官方授权，非商业转载请注明出处。
```

---

参考了[@mike-meng](https://leetcode-cn.com/problems/make-array-strictly-increasing/solution/dp-on2-by-mike-meng/)的解题，时间复杂度为O(n^2lg(n))

除此之外，还有O(N*max(N,M))的解法，比较复杂，以后有时间在看看。

```c++
class Solution {
public:
    int makeArrayIncreasing(vector<int>& arr1, vector<int>& arr2) {
        int len = arr1.size();
        sort(arr2.begin(),arr2.end());

        //dp[i][j]表示，将数组arr1的前j个元素通过i次替换后变为严格递增序列时，序列中最后一个元素的最小值,第j个元素的最小值。
        vector<vector<int>> dp(len+1,vector<int>(len+1,0x3f3f3f3f));
        dp[0][0] = -1;

        for(int i = 1; i <= len; ++i){
            for(int j = 0;j <= len; ++j){
                if(arr1[i-1] > dp[i-1][j]){
                    dp[i][j] = arr1[i-1];
                }
                if(j > 0){
                    auto it = upper_bound(arr2.begin(),arr2.end(),dp[i-1][j-1]);
                    if(it != arr2.end()){
                        dp[i][j] = min(dp[i][j],*it);
                    }
                }
                if( i == len && dp[i][j] != 0x3f3f3f3f){
                    return j;
                }
            }
        }
        return -1;
    }
};

```

---

